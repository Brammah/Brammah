{{-- <div class="mb-5 row">
    <div class="col-md-6">
        <label for="parent_id" class="form-label fw-bold">Parent Agent:</label>
        <select class="form-select" name="parent_id" id="parent_id" data-control="select2"
            data-placeholder="select parent agent">
            <option value="" selected>Select Parent Agent</option>
            @foreach ($parentAgents as $parentAgent)
                <option value="{{ $parentAgent->id }}" @selected(old('parent_id', $agent->parent_id) == $parentAgent->id)>
                    {{ $parentAgent->name }}
                </option>
            @endforeach
        </select>
