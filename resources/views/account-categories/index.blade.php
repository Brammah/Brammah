...
@section('header')
    ...
    <ul class="pt-1 breadcrumb breadcrumb-separatorless fw-semibold fs-7">
        ...
        <li class="text-gray-900 breadcrumb-item">Account Categories</li>
    </ul>
@endsection
    ...
    <div class="shadow-sm card">
        <div class="mt-5 card-header">
            <div class="card-title flex-column">
                <h3 class="mb-1 fw-bold">Account Categories</h3>
                <div class="mb-2 text-gray-400 ">List of Account Categories</div>
            </div>

            <div class="my-1 card-toolbar">
                @can('add account category')
                    <a href="{{ route('account-category.create') }}" class="btn btn-primary btn-sm">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete this Account Category?</p>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-sm btn-light-secondary text-dark font-weight-bold hover-scale"
                        data-bs-dismiss="modal">Cancel</button>
                    <form action="#" method="POST" id="form_account_category_delete">
                        @csrf
                        @method('DELETE')
...
@section('js')
    <script>
        $(function() {
            const table = $('#account-categories-table').DataTable({
                processing: true,
                serverSide: true,
                dom: "<'row'" +
                    ...
                    ">",
                ajax: {
                    url: "{{ route('account-category.index') }}",
                    type: "GET"
                },
                buttons: [{
                        extend: "pageLength",
            ...
            const deleteForm = document.getElementById('form_account_category_delete')
