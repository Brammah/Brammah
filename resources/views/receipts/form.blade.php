...
<div class="mb-5 row">
    ...
    <div class="col-md-6">
        ...
        <input type="date" name="payment_date" class="mb-2 form-control" placeholder="select date" id="payment_date"
            value="{{ old('payment_date', $receipt->payment_date) }}">
        @error('payment_date')
            <span class="fv-plugins-message-container invalid-feedback"><strong>{{ $message }}</strong></span>
        @enderror
    </div>
...
<div class="mb-5 row">
    <div class="col-md-6">
        <label for="customer_id" class="required form-label fw-bold">Customer:</label>
        <select class="form-control form-select @error('customer_id') is-invalid @enderror" data-control="select2"
            data-placeholder="select customer" name="customer_id" id="customer_id">
            <option></option>
            @foreach ($customers as $customer)
                <option value="{{ $customer->id }}" @selected(old('customer_id', $receipt->customer_id) == $customer->id)>
                    {{ $customer->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="col-md-6">
        ...
        <select class="form-control form-select @error('payment_method_id') is-invalid @enderror" data-control="select2"
            ...
            @foreach ($paymentMethods as $paymentMethod)
                <option value="{{ $paymentMethod->id }}" @selected(old('payment_method_id', $receipt->payment_method_id) === $paymentMethod->id)>
...
<div class="mb-5 row d-none" id="cashPaymentContainer">
    ...
    <div class="col-md-6">
        ...
        <input type="number" step="any" name="cash_amount" class="mb-2 form-control"
            placeholder="enter cash amount" id="cash_amount" value="{{ old('cash_amount', $receipt->cash_amount) }}">
        @error('cash_amount')
            <span class="fv-plugins-message-container invalid-feedback"><strong>{{ $message }}</strong></span>
        @enderror
    </div>
...
<div class="mb-5 row d-none" id="withholdingTaxPaymentContainer">
    <div class="col-md-6">
        <label class="required form-label fw-bold" for="withholding_certificate_number">
            Withholding Certificate Number:</label>
        <input type="text" name="withholding_certificate_number" class="mb-2 form-control"
            placeholder="enter withholding certificate number" id="withholding_certificate_number"
            value="{{ old('withholding_certificate_number', $receipt->withholding_certificate_number) }}">
        @error('withholding_certificate_number')
            <span class="fv-plugins-message-container invalid-feedback"><strong>{{ $message }}</strong></span>
        @enderror
    </div>
    <div class="col-md-6">
        <label class="required form-label fw-bold" for="withholding_amount">Withholding Amount:</label>
        <input type="number" step="any" name="withholding_amount" class="mb-2 form-control"
            placeholder="enter withholding amount" id="withholding_amount"
            value="{{ old('withholding_amount', $receipt->withholding_amount) }}">
...
<div class="mb-5 row d-none" id="mpesaPaymentContainer">
    ...
    <div class="col-md-6">
        ...
        <input type="number" step="any" name="mpesa_amount" class="mb-2 form-control"
            ...
            value="{{ old('mpesa_amount', $receipt->mpesa_amount) }}">
        @error('mpesa_amount')
            <span class="fv-plugins-message-container invalid-feedback"><strong>{{ $message }}</strong></span>
        @enderror
    </div>
...
<div class="mb-5 row d-none" id="chequePaymentContainer">
    <div class="col-md-3">
        ...
        <select class="form-control form-select @error('cheque_type') is-invalid @enderror" data-control="select2"
            ...
            <option></option>
            <option value="post-dated-cheque" @selected(old('cheque_type', $receipt->cheque_type) == 'post-dated-cheque')>
                Post Dated Cheque
            </option>
            <option value="current" @selected(old('cheque_type', $receipt->cheque_type) == 'current')>
                Current Cheque
            </option>
        </select>
        @error('cheque_type')
